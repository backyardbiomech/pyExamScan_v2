...
class BuildMigrationGoldenFixture(unittest.TestCase):
    ...
    def test_five_question_types_survive_parsing(self):
        # MC, MA, SA, MD (2 dropdowns), TF -> 6 bubble/open question slots.
        # ES, MB, MT, OR, CT, and both HS blocks are not supported by
        # parser.py today and are dropped with a warning.
    ...
    def test_md_question_points_bug_preserved(self):
        """Known bug, not yet fixed (scoped to phase 3.5): each MD dropdown
        carries the full question point value, so a 1-point MD question with
        two dropdowns is worth 2 points in the key. exam_key_writer.py must
        reproduce this exactly, not silently fix it mid-move."""
        data = exam_key_writer.build_key_data(self.version, 1.0)
        md_dropdown_points = [
            pts for qk, pts in data['point_values'].items()
            if qk in ('Q004', 'Q005')
        ]
        self.assertEqual(md_dropdown_points, [1.0, 1.0])
